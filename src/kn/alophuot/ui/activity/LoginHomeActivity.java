package kn.alophuot.ui.activity;

import java.text.SimpleDateFormat;
import java.util.Calendar;
import java.util.Locale;

import kn.alophuot.app.AloPhuotApplication;
import kn.alophuot.dto.CardInfomation;
import kn.alophuot.service.DeviceRegistrar;
import kn.alophuot.util.ComplexPreferences;
import kn.alophuot.util.Util;
...
public class LoginHomeActivity extends Activity implements OnClickListener {
	private AloPhuotApplication aloPhuotApplication;
	private final String TAG = "LoginHomeActivity";
	private EditText txtUserName, txtPassword;
	private Button btnLogin;
	private TextView tvBreakLogin, tvCannotLogin, tvDonotHaveCard;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		aloPhuotApplication = (AloPhuotApplication) this.getApplication();
		...
		final Typeface mFont = Typeface.createFromAsset(getApplicationContext()
				.getAssets(), "SFUHelveticaLight.ttf");
		...
		tvDonotHaveCard = (TextView) findViewById(R.id.tvDonotHaveCard);
		// get shared preferend card info login
		ComplexPreferences pref = aloPhuotApplication.getComplexPreference();
		if (pref != null) {
			CardInfomation cardInfo = pref.getObject(
					CardInfomation.CARDINFORMATION, CardInfomation.class);
			if (cardInfo != null
					&& cardInfo.getResponseCode().equalsIgnoreCase("0")) {
				...
				txtUserName.setText(numberCardOrEmailValue);
				txtPassword.setText(dateOfBirthValue);
				Intent iMainActivity = new Intent(LoginHomeActivity.this,
						MainActivity.class);
		...
		tvDonotHaveCard.setOnClickListener(this);
	...
	Handler handlerResponseCardInfo = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			cardInfo = (CardInfomation) msg.getData().get(
					CardInfomation.CARDINFORMATION);
			if (cardInfo == null) {
				// new Process
			} else {
				// login success
				if (cardInfo.getResponseCode().equalsIgnoreCase("0")) {
					...
					ComplexPreferences pref = aloPhuotApplication
							.getComplexPreference();
					if (pref != null) {
					...
					if (cardInfo.getResponseCode().equalsIgnoreCase("1")) {
						Toast.makeText(LoginHomeActivity.this,
								R.string.login_wrong_info, Toast.LENGTH_SHORT)
								.show();
					} else if (cardInfo.getResponseCode().equalsIgnoreCase("2")) {
						Toast.makeText(LoginHomeActivity.this,
								R.string.info_not_availeble, Toast.LENGTH_SHORT)
								.show();
					} else if (cardInfo.getResponseCode().equalsIgnoreCase("3")) {
						Toast.makeText(LoginHomeActivity.this,
								R.string.card_not_access, Toast.LENGTH_SHORT)
								.show();
					}
				}
			}
	...
	private void updateTxtDateOfBirth() {
		...
		txtPassword.setText(sdf.format(mCalendar.getTime()));
	}
	...
	public void onClick(View v) {
		...
		case R.id.btnLogin:
			// String cardNumberOrEmailValue =
			// txtNumberCardEmail.getText().toString();
			...
			// new GetLogin(handlerResponseCardInfo, cardNumberOrEmailValue,
			// dateOfBirthValue).run();
			// } else {
			// Toast.makeText(LoginHomeActivity.this,
			// "Your device is not connect network", Toast.LENGTH_LONG)
			// .show();
			// }
	...
	private void registerDevice(String deviceId) {
		...
		NetworkInfo netInfo = connectMgr.getActiveNetworkInfo();
		if (netInfo != null && netInfo.isConnected()) {
			new DeviceRegistrar(aloPhuotApplication, deviceId).run();
		} else {
			Toast.makeText(LoginHomeActivity.this,
					"Your device is not connect network", Toast.LENGTH_LONG)
					.show();
